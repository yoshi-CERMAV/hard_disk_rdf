...
int test_move(position_structure &pos){
    ...
    if( new_address != pos.address) {
        if(address_keeper[new_address] != -1) return 0;
    }
    for(int j = -2; j != 3; j++){
 ...
 //           if(check_i(i, ix, j1* address_lda)) return 0;
             int i1 = pbc(ix + i, jump_x);
             int address = i1 + j1;
             if(address == pos.address) continue;
             int k =address_keeper[address];
             if( k != -1 ) {
                 if(distance_check(k))return 0;
             }
        }
    }
/*    int j1 = pbc(iy-2, jump_y)* address_lda ;
    int j2 = pbc(iy+2, jump_y)* address_lda ;
    for(int i = -1; i !=2; i++){
        if(check_i(i, ix, j1)) return 0;
        if(check_i(i, ix, j2)) return 0;
    }
 */
    accept_count++;
    pos.x = new_x;
